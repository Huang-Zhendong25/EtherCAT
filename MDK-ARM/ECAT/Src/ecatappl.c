...
SDO_Digits RecDigits = {0x3f, 0x3f, 0x3f, 0x51};
//uint8_t FirstChange = 0;
uint8_t OriginalAddr = 0;


uint8_t StateValue = 0;
UINT32 CounterValue = 0;
UINT32 TestValue = 0x0;
UINT32 TestValue1 = 0x0;
UINT32 TestValue2 = 0x0;
UINT32 TestValue3 = 0x0;
uint8_t ConfValue = 0;
...
//YXY_CHANGE:32BIT
//CHANGE_START
#if defined(USE_32BIT) && (USE_32BIT == 1)
...
/*ECATCHANGE_START(V5.12) ECAT1*/
   ...
   if (MEASUREMENT_ACTIVE)
   {
      ...
      u32TimeValue = GetSystemTimeDelay(0);   //NOTE: Calculates the difference between the old and current system time value in ns.
		  u32MinCycleTimeStartValue = u32TimeValue;  //NOTE: timeout counter in ms to measure the process timings
...
/*ECATCHANGE_START(V5.12) ECAT1*/
    ...
    if (MEASUREMENT_ACTIVE)
    {
       u32TimeValue = GetSystemTimeDelay(u32TimeValue);   //NOTE: u32TimeValue : Old system time (0x910:0x913) value 
...
void PDI_Isr(void)
{
	//Test
	/*
	HW_EscWriteDWord(TestValue1, 0x0f80);
	TestValue1 += 1;
	...
	*/
		...
		//EXTINT_TOGGLE();
    if(bEscIntEnabled)   //NOTE: Indicates that the ESC interrupt is enabled
    {
        /* get the AL event register */
        UINT16  ALEvent = HW_GetALEventRegister_Isr();
        ALEvent = SWAPWORD(ALEvent);
				
        if ( ALEvent & PROCESS_OUTPUT_EVENT )   //NOTE: SM2 interrupt pending
        {
            if(bDcRunning && bDcSyncActive) //YXY_NOTE: bDcRunning = TRUE Sync0 events are received; 
            {
            ...
            if(sSyncManOutPar.u16SmEventMissedCounter > 0)
            {
                ...
                sSyncManOutPar.u16SmEventMissedCounter--;
							//Test
							//HW_EscWriteDWord(sSyncManOutPar.u16SmEventMissedCounter, 0x0f94);
            }
...
/*ECATCHANGE_END(V5.12) ECAT5*/
        ...
        */
        if ( bEcatOutputUpdateRunning )    //NOTE:Indicates the OP state, be set in SAFEOP to OP, reset in OP to SAFEOP
        {
        ...
        else
        {
					...
					/*
					UINT32 intMask = 0x93;
					HW_EscWriteDWordIsr(intMask, 0xf80);
					*/
...
//CHANGE_START
#if defined(USE_32BIT) && (USE_32BIT == 1)
            HW_EscReadWordIsr(u32dummy,nEscAddrOutputData);
            HW_EscReadWordIsr(u32dummy,(nEscAddrOutputData+nPdOutputSize-4));
#else
        ...
        */
        if (sSyncManOutPar.u16SyncType == SYNCTYPE_SM_SYNCHRON)
        {
            ...
            ECAT_Application();
				...
				//NOTE: bEcatInputUpdateRunning : SAFEOP to OP state
...
{
	//SYNCOUT0_TOGGLE();
	Sync0WdCounter = 0;     //NOTE: Sync0 watchdog counter

    if(bDcSyncActive)     //NOTE: The Distributed Clocks synchronization is active
    {

        if ( bEcatInputUpdateRunning )    //NOTE: SAFEOP to OP state
        {
            LatchInputSync0Counter++;
        }

        if(u16SmSync0Value > 0)   //Allowed Sync0 events within one SM cycle
        {
           /* Check if Sm-Sync sequence is invalid */
           if (u16SmSync0Counter > u16SmSync0Value)  //NOTE: u16SmSync0Counter: Incremented by one on every Sync0 event and reset to 0 on every SM event.
           {
              if ((nPdOutputSize > 0) && (sSyncManOutPar.u16SmEventMissedCounter <= sErrorSettings.u16SyncErrorCounterLimit))
              {
                 ...
                 sSyncManOutPar.u16SmEventMissedCounter = sSyncManOutPar.u16SmEventMissedCounter + 3;
								...
								//Test
								//HW_EscWriteDWord(sSyncManOutPar.u16SmEventMissedCounter, 0x0f98);
								//sSyncManOutPar.u16SmEventMissedCounter = sSyncManOutPar.u16SmEventMissedCounter + 1;
              }
           ...
           if ((nPdOutputSize == 0) && (nPdInputSize > 0))
           {
              ...
              if ((ALEvent & PROCESS_INPUT_EVENT) == 0)  //NOTE: No SM3 interrupt
              {
              ...
              else  //NOTE: SM3 interrupt pending
              {
        ...
        ECAT_Application();
				...
				//NOTE: bEcatInputUpdataRunning: Indicates the SAFEOP or OP state
        if ( bEcatInputUpdateRunning 
...
*////////////////////////////////////////////////////////////////////////////////////////
extern volatile BOOL Global_Int;
void MainLoop(void)
{
...
/*ECATCHANGE_END(V5.12) COE4*/
       ...
       CheckIfEcatError();
		...
		/*TM1650 Change*/
		uint16_t EtherCATAddr;
		
		HW_EscReadWord(EtherCATAddr, 0x0010);
		HW_EscWriteWord(EtherCATAddr, 0x0f80);
		EtherCATAddr %= 100;
		if(EtherCATAddr != OriginalAddr)
		{
			OriginalAddr = EtherCATAddr;
			
			TM1650_SetDigits(EtherCATAddr);
		}
		else
		{}
		...
		/*
		if(FirstChange == 0)
			if(Digits_SDO0x8010.Light != 0)
			{
				uint8_t digits[3] = {RecDigits.Dig0, RecDigits.Dig1, RecDigits.Dig2};
				Set_Digits(&hi2c2, digits, RecDigits.Light);
				FirstChange = 1;
				HAL_GPIO_WritePin(GPIOA, GPIO_PIN_2, GPIO_PIN_RESET);
			}
		if(FirstChange == 1)
		{
			if(RecDigits.Dig0 != Digits_SDO0x8010.Dig0 || RecDigits.Dig1 != Digits_SDO0x8010.Dig1 || RecDigits.Dig2 != Digits_SDO0x8010.Dig2 || RecDigits.Light != Digits_SDO0x8010.Light)
			{
				uint8_t digits[3] = {Digits_SDO0x8010.Dig0, Digits_SDO0x8010.Dig1, Digits_SDO0x8010.Dig2};
				
				TM1650_Update(digits, Digits_SDO0x8010.Light);
				
				RecDigits.Dig0 = Digits_SDO0x8010.Dig0;
				RecDigits.Dig1 = Digits_SDO0x8010.Dig1;
				RecDigits.Dig2 = Digits_SDO0x8010.Dig2;
				RecDigits.Light = Digits_SDO0x8010.Light;
...
//		
//		HW_EscReadDWord(AddressValue, 0x0010);
//		HW_EscWriteDWord(AddressValue, 0x0f80);
		
		/*
		if(Digits_SDO0x8010.CONF == 1)
		{
			if(RecDigits.Dig0 != Digits_SDO0x8010.Dig0 || RecDigits.Dig1 != Digits_SDO0x8010.Dig1 || RecDigits.Dig2 != Digits_SDO0x8010.Dig2 || RecDigits.Light != Digits_SDO0x8010.Light)
			{
				uint8_t digits[3] = {Digits_SDO0x8010.Dig0, Digits_SDO0x8010.Dig1, Digits_SDO0x8010.Dig2};
		...
		else if(Digits_SDO0x8010.CONF == 0)
		{
			if(FirstChange == 0)
			{
				...
				W25X10CL_ReadData(0x000000, DisplayData, 4);
				
				Digits[0] = DisplayData[0];
				Digits[1] = DisplayData[1];
				Digits[2] = DisplayData[2];
				Light = DisplayData[3];
				TM1650_SetDigits(&hi2c2, Digits, Light);
...
uint8_t state = 0;
/* TIM Callback  ----------------------*/
//YXY_CHANGE:HW_ECAT_TIM_HANDLER
//CHANGE_START
void F_TIM8_IRQHandler(void)//1ms
{
	ECAT_CheckTimer();
}
//CHANGE_END
