...
//YXY_ERROR
//uint16_t temp;
//uint16_t TransCounter;
//uint16_t ECATTransCounter;
//uint16_t TEMP1C32SYNC;
//#include "led.h"
//extern int time1;
//extern int time2;
//extern int time3;
...
//Test
//UINT32 MissedCounterS = 0;
//UINT32 MissedCounterB = 0;
//#include "hardware.h"
...
/*ECATCHANGE_END(V5.12) ECAT1*/
    ...
    /* If no free run is supported the EscInt is always enabled*/
        if (( SyncType0x1C32 != SYNCTYPE_FREERUN ) || ( SyncType0x1C33 != SYNCTYPE_FREERUN ))
        {
        /* ECAT Synchron Mode, the ESC interrupt is enabled YXY_NOTE:PREOP_2_SAFEOP*/
					bEscIntEnabled = TRUE;//YXY_NOTE:SAFEOP
    }
        ...
        {
					/* slave is running in DC-mode YXY_NOTE:PREOP_2_SAFEOP */
            bDcSyncActive = TRUE;
...
/*The application ESM function is separated from this function to handle pending transitions*/
    ...
    Sync1WdCounter = 0;
    Sync1WdValue = 0;
    bDcRunning = FALSE;//YXY_NOTE: bDcRunning = FALSE Sync0 events are not received;
    bSmSyncSequenceValid = FALSE; //YXY_NOTE: bSmSyncSequenceValid = FALSE SM/Sync0 sequence is valid
...
/*ECATCHANGE_START(V5.12) ECAT5*/
    ...
    sSyncManOutPar.u16SmEventMissedCounter = 0;   //StartInputHandler
...
/*ECATCHANGE_START(V5.12) ECAT5*/
    ...
    sSyncManOutPar.u16SmEventMissedCounter = 0;  //StartOutputHandler, SAFEOP to OP
    sSyncManOutPar.u8SyncError = 0;
...
/*The application ESM function is separated from this function to handle pending transitions*/
    ...
    bDcSyncActive = FALSE; //YXY_NOTE: bDcSyncActive = FALSE the Distributed Clocks synchronization is not active
    bDcRunning = FALSE; //YXY_NOTE: bDcRunning = FALSE Sync0 events are not received;
    bSmSyncSequenceValid = FALSE; //YXY_NOTE: bSmSyncSequenceValid = FALSE SM/Sync0 sequence is valid
    u16SmSync0Value = 0;
    u16SmSync0Counter = 0;
    ...
    sSyncManOutPar.u16SmEventMissedCounter = 0;   //StopInputHandler, SAFEOP to PREOP
    sSyncManOutPar.u16CycleExceededCounter = 0;
    sSyncManOutPar.u8SyncError = 0;
            ...
            if ( result == 0 )
            {
                ...
                if(result == 0)
                {
                    /* initialize the AL Event Mask register (0x204) */
									...
									//Test
									/*
									u16ALEventMask |= AL_CONTROL_EVENT;
									u16ALEventMask |= SYNC0_EVENT;
									u16ALEventMask |= MAILBOX_READ_EVENT;
									u16ALEventMask |= MAILBOX_WRITE_EVENT;
									*/
									//Test
    ...
    if(bEcatWaitForAlControlRes)
    {
        ...
        if(EsmTimeoutCounter == 0)
        {
            ...
            switch(nEcatStateTrans)
            {
                ...
                break;
                case SAFEOP_2_OP:
									if(bDcSyncActive)//YXY_NOTE:DC MODE
                    {
                        /*SafeOP to OP timeout expired check which AL status code need to be written*/
                        if(!bDcRunning)//YXY_NOTE: bDcRunning = FALSE Sync0 events are not received
                        {
                            /*no Sync0 signal received*/
                            StatusCode = ALSTATUSCODE_NOSYNCERROR;
                        }
                        else if(!bEcatFirstOutputsReceived && (nPdOutputSize > 0))//YXY_NOTE: bDcRunning = TRUE Sync0 events are received
                        {
...
/*ECATCHANGE_END(V5.12) ESM4*/
		...
		//YXY_ERROR
		//HW_EscWriteWord(temp, SM_STATE_CONTROL_OFFSET1 + (ECATTransCounter-1)*0x06);
...
/*ECATCHANGE_END(V5.12) ECAT5*/
    ...
    if(bDcSyncActive && bEcatInputUpdateRunning)
    {
        ...
        else
        {
            ...
            bDcRunning = TRUE;//YXY_NOTE: bDcRunning = TRUE Sync0 events are received;
        ...
        if(bDcRunning)//YXY_NOTE: bDcRunning = TRUE Sync0 events are received;
        {
        ...
        if(bDcRunning)//YXY_NOTE: bDcRunning = TRUE Sync0 events are received;
        {
           ...
           if(sSyncManOutPar.u16SmEventMissedCounter < sErrorSettings.u16SyncErrorCounterLimit)
            {
							//Test
								//sPVInputs.Symbol0 = sSyncManOutPar.u16SmEventMissedCounter;  //0
//							sPVInputs.Symbol0 += 1;
//								sPVInputs.Synbol2 = 5;
							
                bSmSyncSequenceValid = TRUE; //YXY_NOTE: bSmSyncSequenceValid = TRUE SM/Sync0 sequence is valid  
            ...
            else if (bSmSyncSequenceValid)//YXY_NOTE: bSmSyncSequenceValid = TRUE SM/Sync0 sequence is valid  
            {
							//Test
								//sPVInputs.Symbol1 = sSyncManOutPar.u16SmEventMissedCounter;  //6
//							sPVInputs.Symbol1 += 1;
//								sPVInputs.Synbol2 = 10;
        ...
        else if(bSmSyncSequenceValid)//YXY_NOTE: bDcRunning = FALSE Sync0 events are not received; //YXY_NOTE: bSmSyncSequenceValid = TRUE SM/Sync0 sequence is valid  
        {
           bSmSyncSequenceValid = FALSE; //YXY_NOTE: bSmSyncSequenceValid = FALSE SM/Sync0 sequence is valid
...
void CheckIfEcatError(void)
{
   ...
   if(bDcSyncActive)
   {
       if(bEcatOutputUpdateRunning)
       {
           ...
           else if(!bSmSyncSequenceValid)//YXY_NOTE: bDcRunning = TRUE Sync0 events are received; bSmSyncSequenceValid = FALSE SM/Sync0 sequence is not valid
           {
               AL_ControlInd(STATE_SAFEOP, ALSTATUSCODE_SYNCERROR);
               return;
           }
					 //YXY_NOTE: bSmSyncSequenceValid = TRUE SM/Sync0 sequence is valid  
