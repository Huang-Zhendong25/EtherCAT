...
***AL Status Code*** is provided by TwinCAT, and it's a diagnostic information at the application layer, so it can naturally be found in the EtherCAT Slave Code (ESC). Therefore, the hexadecimal code can be found in the ESC (ecatslv.h specifically) according to AL Staus Code to obtain a simple explanation. Furthermore, in files such as ecatslv.c, some conditional judgements were used to check which conditions met the requirements for generating these hexadecimal codes, thereby inferring the possible causes.<br>
